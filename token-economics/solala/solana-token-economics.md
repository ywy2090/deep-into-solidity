...
- **[一、设计框架](#一设计框架与核心决策)** - 代币机制的核心问题
- **[二、案例研究对比](#二案例研究对比以太坊-vs-solana)** - 两大公链设计哲学对比
- **[三、案例研究：Solana](#三案例研究solana)** - 可预测通胀与高质押率
- **[四、案例研究：以太坊](#四案例研究以太坊ethereum)** - 最小可行发行与通缩模型
- **[五、总结与设计建议](#五总结与设计建议)** - 实用框架和最佳实践
...
代币供应机制不仅仅是技术参数，它是您区块链网络的**经济宪法**。在设计之前，您需要理解它的核心作用：
...
### 1.2 设计前必须回答的 10 个核心问题

在开始设计之前，您需要明确回答以下问题。这些答案将决定您的代币经济模型：

**核心问题速查表：**

| # | 核心问题 | 关键考虑 | 主要选项 | 推荐策略 | 设计影响 |
|---|---------|---------|---------|---------|---------|
| **❶** | **网络定位** | 您的链要解决什么问题？ | • L1 结算层<br>• 高性能执行层<br>• L2 扩容<br>• 应用链 | 根据核心价值主张选择 | 决定通胀策略、质押模型、费用结构 |
| **❷** | **初始分配** | 代币如何公平分配？ | • 公开众筹（ICO）<br>• 预挖+锁定<br>• 公平启动<br>• 空投+奖励 | 社区40-50%、团队15-25%、投资者15-25%、基金会10-15% | 影响去中心化程度和启动信任 |
...
| **❽** | **费用市场** | 如何给区块空间定价？ | • 第一价格拍卖（传统）<br>• EIP-1559（现代） | EIP-1559式：基础费+优先费 | 影响用户体验和价值捕获 |
...
## 二、案例研究对比：以太坊 vs Solana

在深入学习每个案例的细节之前，先通过对比理解两种设计哲学的核心差异。
...
> **学习重点**：可预测递减通胀、高质押率激励、极低费用模型、持续安全预算设计
...
根据第一章的10个核心问题，以下是 Solana 的设计选择和实际实施情况：
...
| **❷** | **初始分配** | 预挖 + 分阶段解锁 | 创世供应: 500M SOL<br>• 投资者: ~26%<br>• 团队/基金会: ~33%<br>• 社区/验证者: ~38% | • 有资金支持开发<br>• 社区份额合理 | • 早期集中度较高<br>• 解锁期带来抛压 |
...
| 功能 | 说明 |
|------|------|
| **交易费用** | 用户支付交易和智能合约执行费用
| **质押奖励** | 质押SOL保护网络安全并获得通胀奖励 |
...
| **非流通供应** | ~53,153,427 SOL | 锁定、质押中的 SOL（8.6%） |
| **质押量** | ~425,323,706 SOL | ） |
...
- epoch_duration_in_years
  - epoch的时间长度, 以年为单位, 计算方式 ( 400 ms *43200 ) / ( 365.25 year* 3600 * 24 )
...
```text
Epoch 总奖励 = 600M × 4.04% × (2/365) ≈ 132,821 SOL
...
> **学习重点**：最小可行发行（MVI）理念、EIP-1559 燃烧机制、PoS 质押模型、通缩驱动的价值捕获
...
| **❷** | **初始分配** | 公开众筹 ICO | 创世供应: 72M ETH<br>• ICO投资者: ~83%<br>• 基金会/团队: ~17%<br>无私募轮次 | • 公平分配<br>• 去中心化程度高<br>• 社区信任强 | • 早期无充足资金<br>• 开发速度受限 |
...
**ETH** 是以太坊区块链的原生代币，在网络中扮演多重角色：

| 功能 | 说明 |
|------|------|
| **交易费用（Gas）** | 用户支付交易和智能合约执行的 Gas 费用 |
| **质押奖励** | 质押 32 ETH 成为验证者保护网络并获得奖励 |
| **网络安全** | 通过权益证明（PoS）共识机制保障网络安全 |
| **价值储存** | 作为去中心化金融（DeFi）生态的基础资产 |
| **智能合约燃料** | 执行智能合约所需的计算资源定价单位 |

**关键特性：**

- ✅ **无预挖供应上限**：没有固定的最大供应量
- ✅ **动态供应**：The Merge 后转为通缩或微通胀
- ✅ **EIP-1559 燃烧**：每笔交易的基础费用被永久销毁
- ✅ **质押激励**：验证者获得区块奖励和交易费用
- ✅ **超声波货币**：目标是成为通缩资产（Ultrasound Money）

#### 1.2 核心设计理念

以太坊的代币经济学围绕以下核心原则设计：

##### **1. 最小可行发行量（Minimum Viable Issuance）**
...
│  • 区块奖励（共识层）： ~4% APY             │
│  • 交易费用（执行层）： 优先费 + MEV        │
│  • MEV 收入： 通过 MEV-Boost 获取额外收益   │
└─────────────────────────────────────────────┘
```

发行量设计为刚好足够激励验证者保护网络，避免过度通胀。

##### **2. 通缩压力机制**

- **EIP-1559 燃烧**：每笔交易的基础费用被销毁
- **网络使用驱动**：使用越多，燃烧越多
- **超声波货币愿景**：燃烧量 > 发行量 → 净通缩
...
- **32 ETH 门槛**：成为独立验证者的最低要求
- **惩罚机制**：罚没（Slashing）和怠工惩罚确保诚实行为
- **激励曲线**：质押率越低，APY 越高，激励参与

#### 1.3 关键指标

**当前供应数据**（基于 2024-2025 数据）：

| 指标 | 数值 | 说明 |
|------|------|------|
| **总供应量** | ~120,500,000 ETH | The Merge 后供应增长显著放缓 |
| **质押量** | ~34,000,000 ETH | 约 28% 的总供应被质押 |
| **质押率** | ~28% | 总供应中参与质押的比例 |
| **活跃验证者** | ~1,060,000+ | 每个验证者质押 32 ETH |
| **年化发行率** | ~0.5-0.8% | 取决于质押参与率 |
| **年化燃烧率** | ~0.3-2.0%+ | 取决于网络使用率 |
| **净通胀率** | -0.2% 至 +0.5% | 动态变化，可能通缩或微通胀 |
...
| 分类 | 说明 | 流动性 |
|------|------|--------|
| **流通供应** | 可在市场自由交易的 ETH | ✅ 完全流通 |
| **质押中** | 在信标链质押的 ETH | ⚠️ 需解质押后可流通 |
| **智能合约锁定** | DeFi 协议中的 ETH | ⚠️ 部分流动（如流动性质押） |
| **已燃烧** | 通过 EIP-1559 永久销毁的 ETH | ❌ 永久移除 |
