...
- [Optimism Bedrock 合约源码完整分析报告](#optimism-bedrock-合约源码完整分析报告)
  ...
  - [术语表](#术语表)
  - [推荐阅读导引](#推荐阅读导引)
    - [阶段一：建立全局心智模型](#阶段一建立全局心智模型)
    - [阶段二：掌握核心用户路径](#阶段二掌握核心用户路径)
    - [阶段三：补足 ERC20 与桥接抽象](#阶段三补足-erc20-与桥接抽象)
    - [阶段四：进入故障证明系统](#阶段四进入故障证明系统)
    - [场景一：源码追踪](#场景一源码追踪)
    - [场景二：快速掌握重点](#场景二快速掌握重点)
  - [系统定位与整体架构](#系统定位与整体架构)
    - [核心合约类图](#核心合约类图)
  - [代码目录与模块详解](#代码目录与模块详解)
  ...
  - [核心调用链与执行路径](#核心调用链与执行路径)
    - [L1 → L2 ETH 存款路径](#l1--l2-eth-存款路径)
      - [ETH 存款：前置条件](#eth-存款前置条件)
      - [ETH 存款：调用链](#eth-存款调用链)
      - [ETH 存款：时序图](#eth-存款时序图)
      - [ETH 存款：参数编码概览](#eth-存款参数编码概览)
      - [ETH 存款：参数编码明细](#eth-存款参数编码明细)
        - [op-node 解析流程](#op-node-解析流程)
      - [ETH 存款：关键结论](#eth-存款关键结论)
      - [ETH 存款：特殊流程](#eth-存款特殊流程)
      - [ETH 存款：源码锚点](#eth-存款源码锚点)
    - [L1 → L2 ERC20 存款路径](#l1--l2-erc20-存款路径)
      - [ERC20 存款：前置条件](#erc20-存款前置条件)
      - [L1 原生代币路径（Lock -\> Mint）](#l1-原生代币路径lock---mint)
      - [L2 原生代币路径（Burn -\> Release）](#l2-原生代币路径burn---release)
      - [ERC20 存款：特殊流程](#erc20-存款特殊流程)
      - [ERC20 存款：源码锚点](#erc20-存款源码锚点)
      - [ERC20 存款：关键结论](#erc20-存款关键结论)
    ...
    - [DepositTx 生命周期（L1 事件、op-node 派生与 L2 执行）](#deposittx-生命周期l1-事件op-node-派生与-l2-执行)
      - [阶段一：L1 事件发射](#阶段一l1-事件发射)
      - [阶段二：op-node 解析与 DepositTx 构建](#阶段二op-node-解析与-deposittx-构建)
      - [DepositTx 生命周期：调用链](#deposittx-生命周期调用链)
      - [DepositTx 生命周期：关键差异](#deposittx-生命周期关键差异)
      - [阶段三：Engine API 交互](#阶段三engine-api-交互)
      - [阶段四：执行引擎内部处理](#阶段四执行引擎内部处理)
      - [DepositTx 生命周期：时序图](#deposittx-生命周期时序图)
      - [DepositTx 生命周期：设计要点](#deposittx-生命周期设计要点)
      - [DepositTx 生命周期：源码锚点](#deposittx-生命周期源码锚点)
      - [DepositTx 生命周期：关键结论](#deposittx-生命周期关键结论)
    - [流程相关核心合约总览](#流程相关核心合约总览)
  - [推荐学习路径](#推荐学习路径)
    - [第一阶段：理解核心架构（必修）](#第一阶段理解核心架构必修)
      - [基础类型与编码：`Types.sol + Hashing.sol + Encoding.sol`](#基础类型与编码typessol--hashingsol--encodingsol)
      - [Portal 核心入口：`OptimismPortal2.sol`（⭐⭐⭐）](#portal-核心入口optimismportal2sol)
      - [系统配置中心：`SystemConfig.sol`（⭐⭐⭐）](#系统配置中心systemconfigsol)
      - [全局安全控制：`SuperchainConfig.sol`](#全局安全控制superchainconfigsol)
    - [第二阶段：跨链消息传递（重要）](#第二阶段跨链消息传递重要)
      - [跨链消息基类：`CrossDomainMessenger.sol`（⭐⭐⭐）](#跨链消息基类crossdomainmessengersol)
      - [L1/L2 Messenger 实现：`L1CrossDomainMessenger.sol + L2CrossDomainMessenger.sol`](#l1l2-messenger-实现l1crossdomainmessengersol--l2crossdomainmessengersol)
      - [提款消息存储：`L2ToL1MessagePasser.sol`](#提款消息存储l2tol1messagepassersol)
    - [第三阶段：桥接系统（重要）](#第三阶段桥接系统重要)
      - [桥接基类：`StandardBridge.sol`（⭐⭐）](#桥接基类standardbridgesol)
      - [L1/L2 Bridge 实现：`L1StandardBridge.sol + L2StandardBridge.sol`](#l1l2-bridge-实现l1standardbridgesol--l2standardbridgesol)
      - [映射代币与工厂：`OptimismMintableERC20.sol + Factory`](#映射代币与工厂optimismmintableerc20sol--factory)
    - [第四阶段：故障证明系统（高级 ⭐⭐⭐）](#第四阶段故障证明系统高级-)
      - [争议游戏工厂：`DisputeGameFactory.sol`](#争议游戏工厂disputegamefactorysol)
      - [锚定状态注册表：`AnchorStateRegistry.sol`](#锚定状态注册表anchorstateregistrysol)
      - [故障争议游戏核心：`FaultDisputeGame.sol`（⭐⭐⭐）](#故障争议游戏核心faultdisputegamesol)
      - [Cannon 虚拟机：`MIPS64.sol + PreimageOracle.sol`（⭐⭐）](#cannon-虚拟机mips64sol--preimageoraclesol)
    - [第五阶段：L2 运行时（进阶）](#第五阶段l2-运行时进阶)
      - [L1 状态注入：`L1Block.sol`](#l1-状态注入l1blocksol)
      - [L2 费用计算：`GasPriceOracle.sol`](#l2-费用计算gaspriceoraclesol)
      - [费用金库体系：`FeeVault` 系列（`BaseFeeVault / L1FeeVault / SequencerFeeVault`）](#费用金库体系feevault-系列basefeevault--l1feevault--sequencerfeevault)
    - [第六阶段：跨链互操作（前沿）](#第六阶段跨链互操作前沿)
      - [L2 间消息收件与中继：`CrossL2Inbox.sol + L2ToL2CrossDomainMessenger.sol`](#l2-间消息收件与中继crossl2inboxsol--l2tol2crossdomainmessengersol)
      - [Superchain 代币桥：`OptimismSuperchainERC20.sol + SuperchainTokenBridge.sol`](#superchain-代币桥optimismsuperchainerc20sol--superchaintokenbridgesol)
    - [第七阶段：部署与治理（可选）](#第七阶段部署与治理可选)
      - [部署编排中心：`OPContractsManager.sol`](#部署编排中心opcontractsmanagersol)
      - [治理代币与铸造管理：`GovernanceToken.sol + MintManager.sol`](#治理代币与铸造管理governancetokensol--mintmanagersol)
      - [治理多签安全模块：Safe 模块系列](#治理多签安全模块safe-模块系列)
    - [第八阶段：基础设施（参考）](#第八阶段基础设施参考)
      - [代理升级体系：`Proxy.sol + ProxyAdmin.sol`](#代理升级体系proxysol--proxyadminsol)
      - [资源计量机制：`ResourceMetering.sol`](#资源计量机制resourcemeteringsol)
      - [通用工具库：`libraries/` 下的工具库](#通用工具库libraries-下的工具库)
...
| ③ | `CrossDomainMessenger.sendMessage()` | 生成版本化 nonce；</br> 计算 `gasLimit = baseGas(_message, _minGasLimit)`；编码 `_data = abi.encodeWithSelector(relayMessage.selector, nonce, sender, target, value, minGasLimit, _message)` |
