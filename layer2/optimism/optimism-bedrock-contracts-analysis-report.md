...
- [Optimism Bedrock 合约源码完整分析报告](#optimism-bedrock-合约源码完整分析报告)
  ...
  - [一、架构总览](#一架构总览)
    - [核心合约类图](#核心合约类图)
  - [二、模块详解](#二模块详解)
    - [1. L1 核心合约 (src/L1/)](#1-l1-核心合约-srcl1)
    - [2. L2 核心合约 (src/L2/)](#2-l2-核心合约-srcl2)
    - [3. 跨链通用合约 (src/universal/)](#3-跨链通用合约-srcuniversal)
    - [4. 故障证明 - 争议游戏 (src/dispute/)](#4-故障证明---争议游戏-srcdispute)
    - [5. 故障证明 - Cannon VM (src/cannon/)](#5-故障证明---cannon-vm-srccannon)
    - [6. 基础库 (src/libraries/)](#6-基础库-srclibraries)
    - [7. 治理 (src/governance/)](#7-治理-srcgovernance)
    - [8. L1 合约管理（src/L1/ 与 src/L1/opcm/）](#8-l1-合约管理srcl1-与-srcl1opcm)
    - [9. 安全多签 (src/safe/)](#9-安全多签-srcsafe)
    - [10. 遗留合约 (src/legacy/)](#10-遗留合约-srclegacy)
    - [11. 外围工具 (src/periphery/)](#11-外围工具-srcperiphery)
    - [12. 第三方依赖 (src/vendor/)](#12-第三方依赖-srcvendor)
  - [三、核心流程图](#三核心流程图)
    - [3.1 ETH 存款流程（L1 → L2）](#31-eth-存款流程l1--l2)
      - [3.1.1 调用链](#311-调用链)
      - [3.1.2 时序图](#312-时序图)
    - [3.2 ERC20 存款流程（L1 → L2）](#32-erc20-存款流程l1--l2)
      - [3.2.1 L1 原生代币存款 (Lock -\> Mint)](#321-l1-原生代币存款-lock---mint)
      - [3.2.2 L2 原生代币存款 (Burn -\> Release)](#322-l2-原生代币存款-burn---release)
    - [3.3 提款完整流程（L2 → L1）](#33-提款完整流程l2--l1)
      - [3.3.1 调用链](#331-调用链)
      - [3.3.2 时序图](#332-时序图)
    - [3.4 争议游戏完整流程](#34-争议游戏完整流程)
      - [3.4.1 调用链](#341-调用链)
      - [3.4.2 时序图](#342-时序图)
    - [3.5 DepositTx 完整生命周期（L1 事件 → op-node 构建 → L2 执行）](#35-deposittx-完整生命周期l1-事件--op-node-构建--l2-执行)
    - [3.6 流程涉及的核心合约总结](#36-流程涉及的核心合约总结)
  - [四、学习路径（按优先级排序）](#四学习路径按优先级排序)
    ...
    - [第六阶段：Interop 跨链（前沿）](#第六阶段interop-跨链前沿)
    - [第七阶段：部署与治理（可选）](#第七阶段部署与治理可选)
    - [第八阶段：基础设施（参考）](#第八阶段基础设施参考)
  - [五、统计信息](#五统计信息)
    - [最复杂/最核心的 Top 10 文件](#最复杂最核心的-top-10-文件)
...
| 术语 | 含义 | 在本文中的典型位置 |
| --- | --- | --- |
| `DepositTx` | OP Stack 中由 L1 日志派生出的特殊 L2 交易类型（`type 0x7E`）。它不依赖普通 ECDSA 验签，`from`、`mint`、`value` 等字段由 L1 事件确定。 | `3.1`、`3.5` |
| `L1InfoDepositTx` | 每个 L2 区块开头都会插入的一笔系统 deposit 交易，用来把 L1 区块信息和系统配置写入 L2 的 `L1Block` 预部署合约。 | `3.5.2`、`3.5.4` |
| `TransactionDeposited` | `OptimismPortal2.depositTransaction()` 在 L1 上发出的事件。`op-node` 会扫描该事件并把它解码成用户 `DepositTx`。 | `3.1.3`、`3.5.1` |
| `sourceHash` | 每笔 deposit 的确定性来源标识。用户 deposit 使用 `keccak256(domain=0 \|\| keccak256(l1BlockHash \|\| logIndex))` 计算，保证同一笔 L1 事件在所有节点上得到相同身份。 | `3.1.3`、`3.5.2`、`3.5.6` |
| `epoch` | 一段共享同一个 L1 origin 的 L2 区块区间。新 epoch 的首块会扫描对应 L1 origin 区块 receipts 来派生用户 deposits；同 epoch 后续块通常不会重复扫描。 | `3.5.2`、`3.5.5` |
| `L1 origin` | 某个 L2 区块引用的 L1 区块。它决定该 L2 区块派生时所使用的 L1 上下文，也是区分 epoch 的核心依据。 | `3.5.2`、`3.5.5` |
| `PayloadAttributes` | `op-node` 发送给执行引擎的构块输入模板，包含时间戳、`Transactions`、gas limit 等字段。它先决定“哪些系统 / deposit 交易必须进块”，再由执行引擎据此构造 payload。 | `3.5.2`、`3.5.3` |
| `ExecutionPayload` | 执行引擎根据 `PayloadAttributes` 构建并返回的最终区块执行结果，包含最终交易列表、状态根、区块哈希等。 | `3.5.3`、`3.5.5` |
| `NoTxPool` | `PayloadAttributes` 中的一个开关。为 `true` 时，执行引擎不会从本地 tx pool 再附加普通用户交易，常用于验证者的确定性派生路径。 | `3.5.6` |
| `地址别名` / `Address Aliasing` | 当 L1 合约通过 Portal 发起存款时，`from` 会加上固定偏移量 `0x1111...1111` 映射到 L2，防止 L1 合约地址在 L2 冒充同地址账户。L2 messenger 再通过 `undoL1ToL2Alias` 还原来源。 | `3.1.2`、`3.1.4`、`3.5.4` |
| `Output Root` | L2 状态对 L1 的压缩承诺，通常由状态根、`L2ToL1MessagePasser` 存储根等字段组成。L1 上的提款证明就是基于这个承诺验证某条 L2 消息确实存在。 | `3.3` |
| `L2ToL1MessagePasser` | L2 上记录提款消息的底层预部署合约。提款哈希写入它的存储后，才可能被后续的 output root 和 L1 证明机制引用。 | `2. L2 核心合约`、`3.3` |
| `CrossDomainMessenger` | L1/L2 共享的跨域消息基类，负责 `sendMessage()`、`relayMessage()`、消息哈希、重放保护、失败消息重试等机制。`L1CrossDomainMessenger` 和 `L2CrossDomainMessenger` 都继承它。 | `2. 跨链通用合约`、`3.1`、`3.3` |
| `StandardBridge` | L1/L2 共享的桥接基类，负责 ETH / ERC20 的桥接发起与确认。运行时看到的是 `L1StandardBridge` / `L2StandardBridge`，而很多核心 `finalizeBridge*` 逻辑实现位于这个基类中。 | `2. 跨链通用合约`、`3.1`、`3.2` |
| `OptimismMintableERC20` | 表示“对侧链原生资产”在当前链上的映射 ERC20。其典型语义是：桥到当前链时 `mint`，从当前链桥回去时 `burn`。 | `2. 跨链通用合约`、`3.2` |
| `FaultDisputeGame` | OP Stack 故障证明的核心争议游戏合约。它通过交互式二分法把“某个 output root 是否正确”的争议逐步缩小到可单步验证的粒度。 | `2. 故障证明 - 争议游戏`、`3.4` |
| `AnchorStateRegistry` | 维护锚定状态和“哪些争议游戏仍然有效”的注册表。L1 提款最终确认时，会依赖它判断相关 dispute game 的 claim 是否仍然可信。 | `2. 故障证明 - 争议游戏`、`3.3`、`3.4` |
| `MIPS64` / `PreimageOracle` | Cannon 故障证明中的链上 VM 与其外部数据提供组件。前者负责单步执行验证，后者负责提供执行所需的预映像数据。 | `2. 故障证明 - Cannon VM`、`3.4` |
| `SafeCall.call` | 一个避免复制 returndata 的底层安全调用库函数，常用于 messenger / bridge 的外部调用路径，可降低 gas 风险并防止 returndata bomb 类问题。 | `3.1`、`3.5.4`、`3.5.6` |
| `ConfigUpdate` | `SystemConfig` 发出的配置变更事件。`op-node` 会据此更新派生所需的系统参数，例如 gas limit、fee scalar、batcher 配置等。 | `2. L1 核心合约`、`SystemConfig` 相关说明 |
...
- 先读 `3.4 争议游戏完整流程`
- 再读 `4. 故障证明 - 争议游戏 (src/dispute/)`
- 最后读 `5. 故障证明 - Cannon VM (src/cannon/)`
...
下图展示了核心合约之间的继承关系（实线箭头 `--|>`）和组合/依赖关系（虚线箭头 `..>`），按 L1、L2、通用基类、争议系统、Cannon VM 五个层级组织。
...
| **OptimismPortalInterop.sol** | 744 | OptimismPortal2 的 Interop（跨链互操作）扩展版本，增加了 Super Root 证明支持。 |
...
| **L2ToL2CrossDomainMessenger.sol** | 0x...0023 | 302 | L2↔L2 跨链消息传递合约（Interop）。 |
...
| **PreimageOracle.sol** | 895 | **预映像预言机**。存储 Cannon VM 需要的外部数据（L1/L2 状态、交易数据等）。支持大预映像提案和挑战。 |
...
### 8. L1 合约管理（src/L1/ 与 src/L1/opcm/）
...
> 交互式可视化版本（本地浏览器打开）：[l1-to-l2-encoding-flow.html](./l1-to-l2-encoding-flow.html)
...
**关键结论**：ETH 存款的本质不是 L1 直接把 ETH 推到 L2，而是 L1 先生成一条可验证事件，再由 `op-node + 执行引擎` 在 L2 端确定性重放这次跨域调用。
...
**关键结论**：ERC20 存款的核心不是 Portal 保管代币，而是 `StandardBridge` 根据资产归属决定走“锁定/释放”还是“销毁/铸造映射代币”两条路径。
...
**关键结论**：提款不是一次交易完成，而是“L2 记录消息 -> L1 证明存在 -> L1 在有效争议游戏窗口后执行”的三阶段流程。
...
**关键结论**：争议游戏的目标不是在链上重放整条 L2 链，而是通过交互式二分法把争议压缩到单步 VM 验证，把链上成本控制在可接受范围。

### 3.5 DepositTx 完整生命周期（L1 事件 → op-node 构建 → L2 执行）

前面 3.1–3.2 描述了合约层面的调用链，本节补充 **op-node 侧的处理逻辑**，将 Solidity 事件与 Go 代码串联起来，形成端到端的完整视图。
...
> 交互式可视化版本（本地浏览器打开）：[deposit-tx-lifecycle.html](./deposit-tx-lifecycle.html)
...
#### 3.5.2 阶段二：op-node 解析 → 构建 DepositTx
...
**关键结论**：DepositTx 生命周期的关键在于“事件确定输入、`op-node` 确定模板、执行引擎确定执行结果”，三者共同保证存款不会被 sequencer 任意省略或重写。
...
#### 1. `Types.sol + Hashing.sol + Encoding.sol`（基础类型和编码）

- 了解 `WithdrawalTransaction`、`OutputRootProof` 等核心数据结构
- 理解 deposit 交易和跨域消息的哈希方式
- 预计时间：1-2 小时

#### 2. `OptimismPortal2.sol`（核心入口 ⭐⭐⭐）

- 这是整个系统的心脏，理解存款和提款的完整流程
- 重点函数：`depositTransaction()`、`proveWithdrawalTransaction()`、`finalizeWithdrawalTransaction()`
- 预计时间：3-4 小时

#### 3. `SystemConfig.sol`（系统配置 ⭐⭐⭐）

- 理解所有链上配置参数如何管理
- 理解确定性存储槽设计和 ConfigUpdate 事件机制
- 预计时间：2-3 小时

#### 4. `SuperchainConfig.sol`（安全控制）
...
#### 5. `CrossDomainMessenger.sol`（消息传递基类 ⭐⭐⭐）

- 核心中的核心：`sendMessage()` 和 `relayMessage()` 的完整逻辑
- 理解消息哈希、版本控制、重放保护、失败消息重试
- 预计时间：3-4 小时

#### 6. `L1CrossDomainMessenger.sol + L2CrossDomainMessenger.sol`

- 理解 L1/L2 两侧的差异化实现
- 重点：地址别名机制（`AddressAliasHelper`）
- 预计时间：1-2 小时

#### 7. `L2ToL1MessagePasser.sol`

- 理解提款哈希的存储和 output root 的关系
- 预计时间：1 小时
...
#### 8. `StandardBridge.sol`（桥接基类 ⭐⭐）

- 理解 ETH/ERC20 的桥接逻辑
- 理解“当前链原生资产”与“对侧链原生资产映射代币”的处理差异
- 预计时间：2-3 小时

#### 9. `L1StandardBridge.sol + L2StandardBridge.sol`

- 两侧的具体实现和遗留事件兼容
- 预计时间：1-2 小时

#### 10. `OptimismMintableERC20.sol + Factory`
...
#### 11. `DisputeGameFactory.sol`

- 理解游戏创建、UUID 去重、clone-with-immutable-args 模式
- 预计时间：2 小时

#### 12. `AnchorStateRegistry.sol`

- 理解锚定状态管理和游戏有效性验证链
- 预计时间：2 小时

#### 13. `FaultDisputeGame.sol`（最复杂的合约 ⭐⭐⭐）

- 交互式二分法的完整实现
- 时钟机制、保证金激励、声明树
- 预计时间：6-8 小时

#### 14. `MIPS64.sol + PreimageOracle.sol`（Cannon VM ⭐⭐）
...
#### 16. `GasPriceOracle.sol`

- L2 费用计算的完整逻辑
- 预计时间：1-2 小时

#### 17. `FeeVault` 系列（`BaseFeeVault / L1FeeVault / SequencerFeeVault`）
...
#### 18. `CrossL2Inbox.sol + L2ToL2CrossDomainMessenger.sol`

- L2↔L2 消息传递
- 预计时间：2-3 小时

#### 19. `OptimismSuperchainERC20.sol + SuperchainTokenBridge.sol`
...
#### 20. `OPContractsManager.sol`

- 理解 OP Stack 的部署编排
- 预计时间：3-4 小时

#### 21. `GovernanceToken.sol + MintManager.sol`
...
#### 23. `Proxy.sol + ProxyAdmin.sol`

- 理解代理升级模式
- 预计时间：1 小时

#### 24. `ResourceMetering.sol`
...
| 3 | SuperFaultDisputeGame.sol | 1299 | Interop 故障证明 |
